<template>
  ...
  <div class="tasks" v-if="hydrated">
    <h1>Task Manager</h1>
    <input v-model="taskname" placeholder="Task Name 2018 " class="input">
    ...
    deleteTask(task) {
      this.$apollo.mutate({
        ...
        update: (store, { data: { deleteTask } }) => {
          const data = store.readQuery({ query: ListTasks });
          data.listTasks.items = data.listTasks.items.filter(task1 => task1.id !== deleteTask.id);
          store.writeQuery({ query: ListTasks, data });
        },
        optimisticResponse: {
          __typename: 'Mutation',
